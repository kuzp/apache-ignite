...
public class PackedInteger {
     ...
     *
     * <p>Because the length is stored in the first byte, this method may be called with only the first byte of the
     * packed integer in the given buffer.  This method only accesses one byte at the given offset.</p>
     *
     * @param buf the buffer to read from.
     * @param off the offset in the buffer at which to start reading.
    ...
    /**
     * Writes a packed sorted long integer starting at the given buffer offset and returns the next offset to be
     * written.
     *
     * @param buf the buffer to write to.
     * @param offset the offset in the buffer at which to start writing.

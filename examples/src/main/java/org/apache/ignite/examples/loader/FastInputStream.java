 ...
 *
 * <p>This class extends {@link InputStream} and its <code>read()</code> methods allow it to be used as a standard input
 * stream.  In addition, it provides <code>readFast()</code> methods that are not declared to throw
...
public class FastInputStream extends InputStream {
    ...
    /**
     * Equivalent to <code>skip()<code> but takes an int parameter instead of a long, and does not check whether the
     * count given is larger than the number of remaining bytes.
     *
     * @see #skip(long)
     */

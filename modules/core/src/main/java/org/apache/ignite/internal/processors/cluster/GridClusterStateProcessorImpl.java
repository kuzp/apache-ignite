...
public class GridClusterStateProcessorImpl extends GridProcessorAdapter implements GridClusterStateProcessor {
    ...
    @Override public IgniteInternalFuture<?> changeGlobalState(final boolean activate,
        ...
        BaselineTopology blt = bltConsIds != null ? new BaselineTopology(bltConsIds, false) : null;
        ...
        while (fut == null || fut.isDone()) {
            fut = new GridChangeGlobalStateFuture(UUID.randomUUID(), activate, blt != null, ctx);
    ...
    @Override public void onBaselineTopologyReady() {
        DiscoveryDataClusterState state0 = globalState;

        if (!state0.active())
            return;

        if (state0.baselineTopology() == null)
            return;

        if (state0.baselineTopology().ready())
            return;
    ...
    @Override public ChangeGlobalStateFinishMessage createChangeGlobalStateFinishMessage(ChangeGlobalStateMessage req,
    ...
    private class GridChangeGlobalStateFuture extends GridFutureAdapter<Void> {
         ...
         * @param ctx Context.
         */
        GridChangeGlobalStateFuture(UUID requestId, boolean activate, boolean twoPhase, GridKernalContext ctx) {

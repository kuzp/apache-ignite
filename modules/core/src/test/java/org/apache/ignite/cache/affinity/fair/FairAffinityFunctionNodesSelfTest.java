    ...
    private void checkSequence0(boolean[] seq) throws Exception {
        try {
            ...
            for (boolean start : seq) {
                ...
                ((IgniteKernal)grid(0)).internalCache(DEFAULT_CACHE_NAME).context().affinity().affinityReadyFuture(topVer).get();

                for (int i : started) {
                    if (i != 0) {
                        IgniteEx grid = grid(i);

                        ((IgniteKernal)grid).internalCache(DEFAULT_CACHE_NAME).context().affinity().affinityReadyFuture(topVer).get();

                        info("Grid " + i + ": " + grid.localNode().id());

                        for (int part = 0; part < parts; part++) {
                            List<ClusterNode> firstNodes = (List<ClusterNode>)grid(0).affinity(DEFAULT_CACHE_NAME)
                                .mapPartitionToPrimaryAndBackups(part);

                            List<ClusterNode> secondNodes = (List<ClusterNode>)grid.affinity(DEFAULT_CACHE_NAME)
                                .mapPartitionToPrimaryAndBackups(part);

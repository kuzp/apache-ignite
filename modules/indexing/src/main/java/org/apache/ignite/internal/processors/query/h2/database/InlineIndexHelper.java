...
public class InlineIndexHelper {
    ...
    private int compareAsDateTime(long pageAddr, int off, Value v, int type) {
        ...
        if(PageUtils.getByte(pageAddr, off) == type) {
            long timePart1, timePart2;
            long datePart1, datePart2;

            switch (type) {
                case Value.TIME:
                    timePart1 = PageUtils.getLong(pageAddr, off + 1);
                    timePart2 = ((ValueTime)v.convertTo(type)).getNanos();

                    return fixSort(Long.signum(timePart1 - timePart2), sortType());

                case Value.DATE:
                    datePart1 = PageUtils.getLong(pageAddr, off + 1);
                    datePart2 = ((ValueDate)v.convertTo(type)).getDateValue();

                    return fixSort(Long.signum(datePart1 - datePart2), sortType());

                case Value.TIMESTAMP:
                    ValueTimestamp v0 = (ValueTimestamp) v.convertTo(type);

                    datePart1 = PageUtils.getLong(pageAddr, off + 1);
                    datePart2 = v0.getDateValue();

                    int c = Long.signum(datePart1 - datePart2);

                    if (c == 0) {
                        timePart1 = PageUtils.getLong(pageAddr, off + 9);
                        timePart2 = v0.getTimeNanos();
        ...
        while (cntr1 < len1 && cntr2 < len2) {
            ...
            switch (c >> 4) {
                ...
                /* 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx */
                    ...
                    if (cntr2 == len2)
                        // The string is malformed (partial partial character at end)
